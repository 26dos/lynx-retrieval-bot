...
func init() {
	logging.SetupLogging(logging.Config{
		Format: logging.PlaintextOutput, // or logging.ColorizedOutput / logging.JSONOutput
		Level:  logging.LevelInfo,       // default level
		Stdout: true,                    // output to stdout
		Stderr: false,                   // do not use stderr to avoid garbled output in nohup
		File:   "",                      // do not write logs to a file directly
...
func main() {
	...
	for {
		...
		// Step 1: inside function, we group by client_addr + miner_addr and keep the top 30% in each group
		...
		// Count totals after keeping top 30%
		...
		// Step 2: random sampling within each group (up to 100 items)
		...
		for client, providerDeals := range dealsGrouped {
			for provider, deals := range providerDeals {
				...
				// Shuffle in place and take the first sampleCount items
...
// First group, then sort by claim_id in descending order; keep only the top 30% for each group

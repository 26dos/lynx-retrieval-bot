...
func main() {
	app := &cli.App{
		...
		Action: func(cctx *cli.Context) error {
			...
			// Map MarketDeal to DBClaim (minimum required fields, adapted for util.AddTasks)
			...
			claims := []model.DBClaim{
				{
					// ClaimID/ProviderID/ClientID/Sector cannot be obtained directly from MarketDeal in this scenario, set to 0
					// Only MinerAddr/ClientAddr/DataCID/Size will be used in task dispatching
					ClientAddr: deal.Proposal.Client,           // f1/f3...
					MinerAddr:  deal.Proposal.Provider,         // f0... (provider corresponds to miner ID address)
					DataCID:    deal.Proposal.PieceCID.Root,    // CID used for piece retrieval
					Size:       int64(deal.Proposal.PieceSize), // padded piece size (bytes)
					TermStart:  termStart,                      // from deal
					TermMax:    termMax,                        // End - Start
					TermMin:    0,                              // not available, set to 0
					UpdatedAt:  time.Now().UTC(),
					Meta: map[string]any{
						"deal_id":      dealID, // saved in meta for reference
			...
			// Generate tasks (util.AddTasks now supports []model.DBClaim)
